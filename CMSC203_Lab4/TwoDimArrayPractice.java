...
public class TwoDimArrayPractice extends JFrame
{
   ...
   */
   public void printArray() 
   {
	   ...
	   //nested loop for printing
	    for (int i = 0; i < intArray.length; i++)
	    {
	    	//newline for each new row, and tab to separate the row number from columns
	        System.out.print("\n" + i + "\t"); 
   ...
   */
   public int findMinimum(int column)
   {
	    ...
	    { 
	    	//checks to see if number is lower, if is, minimum is replaced
   ...
   */
   ...
   {
	   //temporary changing count
	    int temp = 0;

	    //nested loop to count
	    for (int i = 0; i < intArray.length; i++) 
	    { 
